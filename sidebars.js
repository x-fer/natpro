module.exports = {
  materijaliSidebar: {
    ...
    '6. Upiti nad intervalima 2': [].map(title => 'upiti-nad-intervalima-2/' + title),
    '7. Algoritmi nad grafovima 1': [].map(title => 'algoritmi-nad-grafovima-1/' + title),
    '8. Algoritmi nad grafovima 2': [].map(title => 'algoritmi-nad-grafovima-2/' + title),
    '9. Matematika': ['prosti-brojevi', 'vazne-formule', 'kineski-teorem-o-ostatcima', 'multiplikativni-inverz'].map(title => 'matematika/' + title),
    '10. Stringovi': [].map(title => 'stringovi/' + title),
